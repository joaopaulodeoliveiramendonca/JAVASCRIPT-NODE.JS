...
`describe()`: Agrupa testes relacionados. Neste caso, estamos agrupando testes para a função soma.

O primeiro parâmetro é uma descrição do que estamos testando, e o segundo parâmetro é uma função que contém os testes.

`it()`: Descreve o comportamento esperado da função ou método. No nosso caso, estamos dizendo que a função soma deve somar corretamente dois números.

`assert.equal()`: A asserção que estamos fazendo aqui é que o resultado da função soma(2, 3) deve ser igual a 5.
