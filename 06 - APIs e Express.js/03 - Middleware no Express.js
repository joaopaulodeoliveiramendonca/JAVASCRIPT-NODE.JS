...
// O que é Middleware?
// Middleware são funções que têm acesso ao objeto de requisição (req), ao objeto de resposta (res) e à função next no ciclo de requisição/resposta.
// O Express usa middleware para realizar várias tarefas, como validação de dados, verificação de autenticação, entre outros.
...
app.use((req, res, next) => {
  // Exibe a data e hora da requisição recebida
...
//Middleware Global: O middleware definido com app.use() será executado para todas as requisições, exibindo a data e hora da requisição.

//Middleware para Rota Específica: O middleware para a rota /clientes verifica se o corpo da requisição contém o campo nome. Se faltar, retorna um erro (status 400). Caso contrário, chama next() para passar o controle para a próxima função (que cria o cliente).
